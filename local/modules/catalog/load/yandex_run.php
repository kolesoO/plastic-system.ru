            ...
            foreach ($itemIdsList as $id)
            {
                ...
                if ($row['TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
                {
                    ...
                    foreach ($row['OFFERS'] as $offer)
                    {
                        ...
                        //offer_id in url
                        $referer .= (!$disableReferers ? '&amp;' : '?') . 'offer_id=' . $offer['ID'];
                        //end

                        $itemsContent .= "<url>".($offerUrlConfig['USE_DOMAIN'] ? $usedProtocol.$site['SERVER_NAME'] : '').htmlspecialcharsbx($row['DETAIL_PAGE_URL']).$referer."</url>\n";
                        unset($referer);
