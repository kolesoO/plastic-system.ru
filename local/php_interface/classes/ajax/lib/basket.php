                ...
                if (\Bitrix\Main\Loader::includeModule("sale")) {
                    ...
                    while ($arItem = $rsItem->fetch()) {
                        if ($arItem["CATALOG_AVAILABLE"] != "Y" || floatval($arItem["CATALOG_PRICE_".$arParams["price_id"]]) == 0) {
                            $arReturn["error_msg"][] = self::getMsg("ITEMS_NOT_AVAILABLE", $arItem["NAME"]);
                            continue;
                        }
                        if ($basketId = \CSaleBasket::Add([
                            ...
                            $arReturn["error_msg"][] = self::getMsg("ADD_TO_BASKET_ERROR", $arItem["NAME"]);
                        }
                    }
                } else {
                    $arReturn["error_msg"][] = self::getMsg("MODULE_SALE_NOT_INSTALLED");
                }
            } else {
                $arReturn["error_msg"][] = self::getMsg("PRICE_TYPE_NOT_FOUND");
            }
        } else {
            $arReturn["error_msg"][] = self::getMsg("ITEMS_NOT_FOUND");
        }
